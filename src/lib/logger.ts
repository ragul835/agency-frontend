...
const logger = {
  info: (msg: string, ...args: unknown[]) => {
    if (isDev) console.info(`%cNexCore%c ${msg}`, styles.prefix, styles.info, ...args);
  },
  warn: (msg: string, ...args: unknown[]) => {
    console.warn(`%cNexCore%c ${msg}`, styles.prefix, styles.warn, ...args);
  },
  error: (msg: string, ...args: unknown[]) => {
    console.error(`%cNexCore%c ${msg}`, styles.prefix, styles.error, ...args);
  },
  route: (path: string) => {
    if (isDev) console.info(`%cNexCore%c navigate → %c${path}`, styles.prefix, "", styles.route);
  },
  api: (method: string, path: string, status: number, ms: number) => {
    const ok = status >= 200 && status < 400;
    const style = ok ? styles.apiOk : styles.apiErr;
    const label = `%cNexCore API%c ${method} ${path} → %c${status} (${ms}ms)`;
    if (ok) {
